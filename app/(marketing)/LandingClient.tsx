...
function listingTag(l: Listing): string {
  if (l.is_furnished) return "Furnished";

  if (l.are_pets_allowed) return "Pet OK";

  if (l.available_date == null) return "Available Now";

  return "New";
}

function FeaturedStays({ listings }: { listings: Listing[] }) {
    ...
    <section className="pt-16 pb-20 border-t border-[#E5DFD2]">
      <div className="mx-auto max-w-[1320px] px-8">
        <div className="flex items-end justify-between mb-9 flex-wrap gap-5">
          <div>
            <Eyebrow className="mb-3.5">Newest · 401 Corridor</Eyebrow>
          ...
          <div className="flex items-center gap-2">
            ...
            >
              View all stays <ArrowRight className="size-3.5" />
            </Link>
          </div>
        </div>
        ...
        >
          {listings.map((l) => {
            ...
            return (
              <Link
                key={l.id}
                href={`/facility-map?listing=${l.id}`}
                className="snap-start flex flex-col gap-3.5 transition-transform duration-300 hover:-translate-y-1 no-underline text-inherit"
              >
                <div
                ...
                <div>
                  <div className="flex justify-between items-baseline gap-3">
                    <h3 className="text-lg font-semibold m-0 tracking-[-0.015em] line-clamp-1">
                      {l.title}
                    </h3>

                    {price != null && (
                      <span className="shrink-0 text-lg font-semibold tracking-[-0.02em]">
                  ...
                  <p className="mt-1 mb-3 text-sm text-[#6B7585] line-clamp-1">
                    {l.address}
                  </p>

                  <div className="flex gap-4 text-[13px] text-[#3A4759]">
                    {l.bedrooms != null && (
                      <span className="inline-flex items-center gap-1.5">
                        <Bed className="size-4" /> {l.bedrooms} bed
                      </span>
                    )}

                    {l.bathrooms != null && (
                      <span className="inline-flex items-center gap-1.5">
                        <Bath className="size-4" /> {l.bathrooms} bath
                      </span>
                    )}

                    {l.square_footage != null && (
                      <span className="inline-flex items-center gap-1.5">
                        <Square className="size-4" /> {l.square_footage} sqft
                      </span>
                    )}
                  </div>
                </div>
